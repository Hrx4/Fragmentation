...
export const BurnPage = () => {
  ...
  const { burnAmount, onChangeBurnAmount } = useBurnAmount();

  //refetchTransaction Hook
  const { burnTransactions, setBurnTransactions, refetchTransactions } =
    useRefetchTransaction();

  //useExecuteBurn Hook

  const { txButton, txProgress , burnTxHash , executeBurn } = useExecuteBurn(
    isWalletConnected,
    walletChain,
    openConnectModal,
    burnAmount,
